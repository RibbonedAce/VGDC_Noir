...
public class PublicFunctions : MonoBehaviour {
    ...
    public static float FindAngle(Vector3 target)
    {
        float result = 0;

        float[] xlist = { Mathf.Rad2Deg * Mathf.Acos(target.x), -Mathf.Rad2Deg * Mathf.Acos(target.x) };
        float[] ylist = { Mathf.Rad2Deg * WrapSin(Mathf.Asin(target.y)), 180 - (Mathf.Rad2Deg * WrapSin(Mathf.Asin(target.y))) };
